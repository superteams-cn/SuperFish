    ...
    "entity_types": [
        {
            ...
            "display_name": "面向用户展示的实体类型名称（使用当前语言，例如中文）",
    ...
    "edge_types": [
        {
            ...
            "display_name": "面向用户展示的关系类型名称（使用当前语言，例如中文）",
            "description": "简短描述（使用当前语言，不超过100字符）",
            "source_targets": [
...
class OntologyGenerator:
    ...
    def generate(
        ...
        system_prompt = f"{ONTOLOGY_SYSTEM_PROMPT}\n\n{lang_instruction}\nIMPORTANT: Entity type names MUST be in English PascalCase (e.g., 'PersonEntity', 'MediaOrganization'). Relationship type names MUST be in English UPPER_SNAKE_CASE (e.g., 'WORKS_FOR'). Attribute names MUST be in English snake_case. The display_name, description, examples, attribute descriptions, and analysis_summary fields MUST use the specified language above."
        messages = [
...
6. 每个实体类型和关系类型都必须包含 display_name，用当前语言写给用户看的名称；name 仍必须保持英文 schema 名
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for entity in result["entity_types"]:
            # 强制将 entity name 转为 PascalCase（图谱 schema 要求）
            if "name" in entity:
            ...
            if not entity.get("display_name"):
                entity["display_name"] = entity.get("name", "")
        ...
        for edge in result["edge_types"]:
            # 强制将 edge name 转为 SCREAMING_SNAKE_CASE（图谱 schema 要求）
            if "name" in edge:
                original_name = edge["name"]
                edge["name"] = original_name.upper()
                if edge["name"] != original_name:
                    logger.warning(f"Edge type name '{original_name}' auto-converted to '{edge['name']}'")
            if not edge.get("display_name"):
                edge["display_name"] = edge.get("name", "")
        ...
        person_fallback = {
            "name": "Person",
            "display_name": fallback_text["person_display"],
        ...
        organization_fallback = {
            "name": "Organization",
            "display_name": fallback_text["organization_display"],
