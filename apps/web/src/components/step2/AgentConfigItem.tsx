    ...
    <div className="rounded-md border bg-card p-3">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">{agent.entity_name}</span>
        </div>
        <div className="flex gap-1">
          <span className="rounded bg-muted px-1.5 py-0.5 text-[10px]">{agent.entity_type}</span>
          {agent.stance && (
            <span className="rounded bg-muted px-1.5 py-0.5 text-[10px]">{agent.stance}</span>
          )}
        </div>
      </div>
      ...
      {/* 24 小时活跃时间轴 */}
      <div className="mb-3">
        <span className="text-[10px] text-muted-foreground">{t('step2.activeTimePeriod')}</span>
      ...
      <div className="grid grid-cols-3 gap-2 text-[11px]">
        ...
        />
        <div>
          <span className="block text-[9px] text-muted-foreground">{t('step2.activityLevel')}</span>
        ...
        <div>
          <span className="block text-[9px] text-muted-foreground">{t('step2.sentimentBias')}</span>
          <span
            className={cn(
              'font-mono font-semibold',
              sentiment > 0 ? 'text-green-600' : sentiment < 0 ? 'text-red-600' : 'text-muted-foreground',
            )}
          >
            {sentiment > 0 ? '+' : ''}
            {sentiment.toFixed(1)}
