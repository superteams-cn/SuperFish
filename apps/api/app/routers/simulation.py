...
def get_graph_entities(graph_id: str, entity_types: str = '', enrich: str = 'true'):
    ...
    try:
        ...
        entity_types_list = [et.strip() for et in entity_types_str.split(',') if et.strip()] if entity_types_str else None
        enrich_bool = enrich.lower() == 'true'

        logger.info(f"获取图谱实体: graph_id={graph_id}, entity_types={entity_types_list}, enrich={enrich_bool}")

        reader = Neo4jEntityReader()
        result = reader.filter_defined_entities(
            graph_id=graph_id,
            defined_entity_types=entity_types_list,
            enrich_with_edges=enrich_bool
        )
...
def get_simulation_posts(simulation_id: str, platform: str = 'reddit', limit: int = 50, offset: int = 0):
    """
    获取模拟中的帖子
...
def get_simulation_comments(simulation_id: str, post_id: str | None = None, limit: int = 50, offset: int = 0):
    """
    获取模拟中的评论（仅 Reddit）
